    ...
    return (
      <div className="w-full min-h-full pb-8">
        <div className="flex items-center justify-center h-64">
          ...
          <span className="ml-3 text-base font-medium text-slate-400">Loading lessons...</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="w-full min-h-full pb-8">
        <div className="bg-red-500/20 border border-red-500/50 rounded-xl p-8 text-center">
          <div className="text-red-400 text-lg font-bold mb-2">Error Loading Lessons</div>
    ...
    <div className="w-full min-h-full pb-8">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-3xl font-bold text-white tracking-tight mb-2">Lessons</h1>
            <p className="text-slate-400 text-base font-light">
              {filteredLessons.length} of {lessons.length} lessons
            </p>
          </div>
          <Link to="/content/lessons/add">
            <button className="px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all font-semibold text-base shadow-xl hover:shadow-2xl">
              Add New Lesson
            </button>
          </Link>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-12 pr-5 py-3 bg-slate-700/40 border border-slate-600/30 rounded-xl text-base text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all"
            />
          </div>
          ...
          <div className="relative">
            ...
            <select
              ...
              className="pl-12 pr-8 py-3 bg-slate-700/40 border border-slate-600/30 rounded-xl text-base text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all appearance-none"
            >
      ...
      {filteredLessons.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredLessons.map((lesson) => (
            <div key={lesson._id} className="bg-slate-700/40 backdrop-blur-sm border border-slate-600/30 rounded-xl shadow-xl hover:shadow-2xl transition-all hover:scale-[1.02]">
              {/* Card Header */}
              <div className="p-6 pb-4">
                ...
                {lesson.practiceQuestions && lesson.practiceQuestions.length > 0 && (
                  <div className="border-t border-slate-600/30 pt-3">
                    <div className="text-xs font-bold text-slate-300 mb-2 uppercase tracking-wide">
                      Practice Questions:
                    </div>
                    <div className="space-y-1">
                      {lesson.practiceQuestions.slice(0, 2).map((question, index) => (
                        <div key={index} className="text-xs text-slate-400 line-clamp-1">
              ...
              {/* Card Footer */}
              <div className="px-6 py-4 bg-slate-800/40 border-t border-slate-600/30 rounded-b-xl">
                <Link to={`/content/lessons/${lesson._id}`}> 
                  <button className="w-full px-4 py-3 bg-blue-600 text-white text-sm font-semibold rounded-xl hover:bg-blue-700 transition-all shadow-lg hover:shadow-xl">
                    View Lesson
                  </button>
                </Link>
              </div>
